...
# ---------------------------------------------------------------------------
# Expense → segment/option link tests
# ---------------------------------------------------------------------------


def test_create_expense_with_segment_id(client, test_user, db_session):
    """Creating an expense linked to a segment stores the segment_id."""
    db = db_session
...
def test_create_expense_with_segment_option_id(client, test_user, db_session):
    """Creating an expense linked to a segment option stores segment_option_id."""
    db = db_session
    ...
    payload = {
        ...
        "date": "2030-01-02",
        "trip_id": trip.id,
        "segment_option_id": option.id,
    }
    resp = client.post("/expenses/", json=payload)
    assert resp.status_code == 201
    data = resp.json()
    assert data["segment_option_id"] == option.id
    assert float(data["amount"]) == 35.0


def test_create_expense_with_stop_option_id(client, test_user, db_session):
    """Creating an expense linked to a stop option stores stop_option_id."""
    db = db_session
    ...
    seg = models.JourneySegment(
        journey_id=journey.id,
        segment_type="stop",
        origin_name="City Park",
        order=0,
    )
    db.add(seg)
    db.commit()

    stop_opt = models.StopOption(
        segment_id=seg.id,
        name="Museum Entry",
    ...
    payload = {
        ...
        "date": "2030-01-02",
        "trip_id": trip.id,
        "stop_option_id": stop_opt.id,
    }
    resp = client.post("/expenses/", json=payload)
    assert resp.status_code == 201
    data = resp.json()
    assert data["stop_option_id"] == stop_opt.id
    assert float(data["amount"]) == 12.0


def test_update_expense_add_segment_option_link(client, test_user, db_session):
    """Updating an existing expense to add a segment_option_id link works."""
    db = db_session
    ...
    resp = client.put(
        f"/expenses/{exp.id}",
        json={"segment_option_id": option.id},
    )
    assert resp.status_code == 200
    assert resp.json()["segment_option_id"] == option.id
