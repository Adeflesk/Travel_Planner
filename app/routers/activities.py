...
def create_activity(
    ...
    if trip_id_for_auth is not None:
        get_trip_with_access(trip_id_for_auth, db, current_user, "edit")
...
def get_trip_activities(
    ...
    get_trip_with_access(trip_id, db, current_user, "view")
...
def get_day_activities(
    ...
    if not day:
        raise HTTPException(status_code=404, detail="Day not found")
    get_trip_with_access(day.trip_id, db, current_user, "view")
    return (
        db.query(models.DayActivity)
...
def get_destination_activities(
    ...
    if not dest:
        raise HTTPException(status_code=404, detail="Destination not found")
    get_trip_with_access(dest.trip_id, db, current_user, "view")
    return (
        db.query(models.DayActivity)
...
def update_activity(
    ...
    trip_id_for_mut, _ = _resolve_trip_and_date(activity, db)
    if trip_id_for_mut is None:
        raise HTTPException(status_code=404, detail="Activity not found")
    get_trip_with_access(trip_id_for_mut, db, current_user, "edit")
...
def delete_activity(
    ...
    activity = _get_activity_or_404(activity_id, db)
    trip_id_for_del, _ = _resolve_trip_and_date(activity, db)
    if trip_id_for_del is None:
        raise HTTPException(status_code=404, detail="Activity not found")
    get_trip_with_access(trip_id_for_del, db, current_user, "edit")
    db.delete(activity)
    db.commit()
