...
test.describe('Journey Segment Wizard', () => {
  ...
  // TODO: These 3 tests need updating — the Road trip intent now routes to the
  // dedicated RoadTripBuilder UI (two-column layout) instead of the step-by-step
  // segment editor, so "1 / 5" and SegmentCard transport-option checks no longer apply.
  test.fixme('should show transport option cards for TRANSFER segments', async ({ authenticatedPage }) => {
    await openWizard(authenticatedPage);
  ...
  test.fixme('should select a transport option card', async ({ authenticatedPage }) => {
    await openWizard(authenticatedPage);
  ...
  test.fixme('should add a custom transport option', async ({ authenticatedPage }) => {
    await openWizard(authenticatedPage);
